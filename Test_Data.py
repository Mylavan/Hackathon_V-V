...
# Define the keys to look for (case-insensitive) from the table
keys_from_table = [
    "Test Folder Number",
    "Start Date",
    "Stop Date",
...
def CleanDatafromtable(raw_data):
   ...
   for item in raw_data:
       ...
       filtered = [p for p in parts if p and p != 'S' and p != 'CNR' and p != 'N/A' and p != 'All']
...
def Table_Extractor_Entry(pdf_path, xml_path):
    ...
    SystemUsedRaw = Specific_table_extractor(xml_path, 1, 3, 3)  # Extract first table from first page ::::: systems_used
    FailureSummaryRaw = Specific_table_extractor(xml_path, 1, 4, 3)  # Extract second table from first page :::::Failure_Summary
    TestEquipmentRaw1 = Specific_table_extractor(xml_path, 1, 5, 6)  # Extract second table from first page :::::Test_Equipment
...
def Test_Data_Extraction( pdf_path):
    ...
    # result["Intials"] = result["See Signature Above (initials)"]
    # result=result+ key_map
    # del result["See Signature Above (initials)"]
...
# Example usage
def Test_Data_sheet_Main():
    pdf_path= r"C:\DEMO_TEST\D001929432 1\PW Doppler Functional - US217F1107 - Affiniti - R1 - VMQ - 13.0.0.1457 - Test Data Sheet.pdf"
